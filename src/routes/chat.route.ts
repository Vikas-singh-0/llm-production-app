 ...
 * Uses LLM Factory for provider-agnostic AI operations.
 */
router.post('/chat', async (req: Request, res: Response) => {
  try {
    ...
    const llmMessages = memoryService.formatForContext(contextMessages, summary);

    // Call LLM API (non-streaming)
    const { text: assistantReply, usage } = await llmFactory.getProvider().chat(llmMessages);
    ...
    if (error instanceof Error && error.message.includes('API key')) {
      res.status(500).json({
        ...
        message: 'LLM API not configured. Please set the appropriate API key.',
      });
      return;
    }
 ...
 * Uses LLM Factory with token streaming.
 */
router.post('/chat/stream', async (req: Request, res: Response) => {
  try {
    ...
    // Stream from LLM
    await llmFactory.getProvider().streamChat(llmMessages, {
      requestId: req.requestId,
      onToken: (token) => {
        fullResponse += token;
