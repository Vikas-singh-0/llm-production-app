...
print_header "Testing Streaming Endpoints (/chat/stream)"
...
print_info "Using chat ID: $STREAM_CHAT_ID for streaming tests"
...
if echo "$body" | grep -q '"usage"'; then
    ...
    # Extract usage info
    usage_line=$(echo "$body" | grep '"usage"' | tail -1)
    if [ -n "$usage_line" ]; then
        print_info "Usage data present in final event"
    fi
else
...
if [ "$status_code" = "200" ]; then
    ...
    # Try to extract chat_id from completion event
    completion_event=$(echo "$body" | grep '"done":true' | tail -1)
    if [ -n "$completion_event" ]; then
        print_info "Completion event received for auto-created chat"
    fi
else
